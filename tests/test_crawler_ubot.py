...
from src.crawlers.banks.ubot import UbotCrawler
...
def test_crawler_creates_bank(db_session):
    crawler = UbotCrawler(db_session)
    bank = crawler.bank
    assert bank.name == "聯邦銀行"
    assert bank.code == "ubot"


def test_detect_card_type():
    crawler = UbotCrawler.__new__(UbotCrawler)
    assert crawler._detect_card_type("賴點卡 御璽卡", "") == "御璽卡"
    assert crawler._detect_card_type("無限卡", "") == "無限卡"
    assert crawler._detect_card_type("普通卡", "") == "白金卡"
