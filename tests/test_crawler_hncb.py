...
from src.crawlers.banks.hncb import HncbCrawler
...
def test_crawler_creates_bank(db_session):
    crawler = HncbCrawler(db_session)
    bank = crawler.bank
    assert bank.name == "華南銀行"
    assert bank.code == "hncb"


def test_detect_card_type():
    crawler = HncbCrawler.__new__(HncbCrawler)
    assert crawler._detect_card_type("紅利御璽卡", "") == "御璽卡"
    assert crawler._detect_card_type("世界卡", "") == "世界卡"
    assert crawler._detect_card_type("享利卡", "") == "白金卡"
